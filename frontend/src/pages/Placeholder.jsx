                    ...
                    <table className="w-full text-left border-collapse min-w-[500px]">
                      <thead>
                        <tr className={`border-b ${isDark ? 'bg-[#0d1310] border-[#1f2924]' : 'bg-surface-container-lowest border-outline-variant'}`}>
                          <th className={`text-[10px] uppercase font-bold tracking-wider whitespace-nowrap ${isDark ? 'text-gray-500' : 'text-on-surface-variant'}`} style={{ padding: '14px 24px' }}>{t('ID')}</th>
                          <th className={`text-[10px] uppercase font-bold tracking-wider whitespace-nowrap ${isDark ? 'text-gray-500' : 'text-on-surface-variant'}`} style={{ padding: '14px 24px' }}>{t('Village')}</th>
                          <th className={`text-[10px] uppercase font-bold tracking-wider whitespace-nowrap ${isDark ? 'text-gray-500' : 'text-on-surface-variant'}`} style={{ padding: '14px 24px' }}>{t('Status')}</th>
                          <th className={`text-[10px] uppercase font-bold tracking-wider whitespace-nowrap text-right ${isDark ? 'text-gray-500' : 'text-on-surface-variant'}`} style={{ padding: '14px 24px' }}>{t('Action')}</th>
                        </tr>
                      </thead>
                      <tbody>
                        {[1, 2, 3].map((row, i, arr) => (
                          <tr key={row} className={`transition-colors ${isDark ? 'hover:bg-[#1a241d]' : 'hover:bg-surface-container-lowest'} ${i !== arr.length - 1 ? (isDark ? 'border-b border-[#1f2924]' : 'border-b border-outline-variant') : ''}`}>
                            ...
                            <td className="whitespace-nowrap" style={{ padding: '18px 24px' }}>
                              <span className={`px-2 py-0.5 text-[11px] rounded font-bold uppercase tracking-wide ${isDark ? 'bg-[#064e3b] text-[#4ade80]' : 'bg-primary-container text-primary'}`}>{t('Verified')}</span>
                            </td>
                            <td className="text-right whitespace-nowrap" style={{ padding: '18px 24px' }}>
                              <button className={`text-[13px] font-bold hover:underline ${isDark ? 'text-[#4ade80]' : 'text-primary'}`}>{t('View')}</button>
                            </td>
                          </tr>
                        ))}
                      </tbody>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4" style={{ padding: '22px 24px' }}>
                    {[1, 2].map(card => (
                      <div key={card} className={`rounded-xl border shadow-sm flex flex-col justify-between ${isDark ? 'bg-[#151f18] border-[#1f2924]' : 'bg-white border-outline-variant'}`} style={{ padding: '20px' }}>
                        <div className={`text-[10px] font-bold uppercase tracking-wider mb-3 leading-none ${isDark ? 'text-gray-500' : 'text-on-surface-variant'}`}>{t('Metric')} {card}</div>
                        ...
                        <div className={`text-[11px] font-semibold flex items-center leading-none ${isDark ? 'text-[#4ade80]' : 'text-primary'}`}>
                          <span className="material-symbols-outlined text-[14px] mr-1">trending_up</span> {t('+2.4% vs last week')}
                        </div>
                      </div>
                    ))}
                  </div>
                    ...
                    {[1, 2, 3].map(item => (
                      <div key={item} className={`flex items-center justify-between rounded-lg border shadow-sm ${isDark ? 'bg-[#151f18] border-[#1f2924]' : 'bg-white border-outline-variant'}`} style={{ padding: '14px 18px' }}>
                        <div className="flex items-center gap-3 min-w-0">
                          ...
                          <span className={`text-[13px] font-medium truncate ${isDark ? 'text-gray-300' : 'text-on-background'}`}>{t('Document_Analysis_')}{item}{t('.pdf')}</span>
                        </div>
                        <span className={`text-[11px] font-medium uppercase tracking-wider whitespace-nowrap ml-4 ${isDark ? 'text-gray-500' : 'text-on-surface-variant'}`}>{t('Today, 10:45 AM')}</span>
                      </div>
                    ))}
                  </div>
                )}
          ...
          <div className={`${isDark ? 'bg-[#111814] border-[#1f2924]' : 'bg-white border-outline-variant'} rounded-[16px] border shadow-sm flex flex-col min-w-0 overflow-hidden`}>
            ...
            <div className="flex flex-col gap-4" style={{ padding: '22px' }}>
              <div className={`flex items-center justify-between border-b pb-4 ${isDark ? 'border-[#1f2924]' : 'border-outline-variant'}`}>
                <span className={`text-[12px] font-medium ${isDark ? 'text-gray-400' : 'text-on-surface-variant'}`}>{t('Data Sync')}</span>
                <span className={`text-[12px] font-bold flex items-center gap-1.5 ${isDark ? 'text-[#4ade80]' : 'text-primary'}`}>
                  <span className={`w-1.5 h-1.5 rounded-full ${isDark ? 'bg-[#4ade80]' : 'bg-primary'}`}></span> {t('Online')}
                </span>
              </div>
              <div className={`flex items-center justify-between border-b pb-4 ${isDark ? 'border-[#1f2924]' : 'border-outline-variant'}`}>
                <span className={`text-[12px] font-medium ${isDark ? 'text-gray-400' : 'text-on-surface-variant'}`}>{t('Last Update')}</span>
                <span className={`text-[12px] font-semibold ${isDark ? 'text-gray-200' : 'text-on-background'}`}>{t('2 mins ago')}</span>
              </div>
              <div className="flex items-center justify-between">
                <span className={`text-[12px] font-medium ${isDark ? 'text-gray-400' : 'text-on-surface-variant'}`}>{t('Access Level')}</span>
                <span className={`text-[12px] font-semibold ${isDark ? 'text-gray-200' : 'text-on-background'}`}>{isDark ? t('Executive Command') : t('Operational Command')}</span>
              </div>
            </div>
          </div>
          ...
          <div className={`${isDark ? 'bg-[#2a1114] border-[#4c1d22]' : 'bg-[#fffbeb] border-[#fde047]'} rounded-[16px] border flex flex-col shadow-sm min-w-0 overflow-hidden`}>
            ...
            <div style={{ padding: '22px' }}>
              <p className={`text-[13px] leading-snug m-0 ${isDark ? 'text-red-300' : 'text-[#92400e]'}`}>
                {t('No critical anomalies detected in current operational sector.')}
              </p>
            </div>
          </div>
        </div>
