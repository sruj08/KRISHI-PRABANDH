...
const RiskBadge = ({ risk }) => {
  const cfg = {
    ...
    CLEAN:    { bg: '#e8f5e9', color: '#2e7d32', label: 'CLEAN' },
...
const TABS = [
  ...
  { id: 'apps',      icon: 'assignment',       label: 'App Intelligence' },
  { id: 'vistar',    icon: 'school',           label: 'Krushi Vistar' },
];
...
const MandalDashboard = () => {
  ...
  if (!currentMandal) return (
    <div style={{ textAlign: 'center', padding: '60px 20px' }}>
      ...
      <h2 style={{ marginTop: '16px', color: '#888' }}>No Mandal Selected</h2>
      <p style={{ color: '#aaa', fontSize: '13px', marginTop: '8px' }}>Select a Mandal from the header to begin supervisory view.</p>
    </div>
  );
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
      ...
      {loading && <div style={{ textAlign: 'center', color: '#aaa', padding: '40px' }}>Loading data…</div>}
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
          ...
          {vistar && (
            <div style={{ background: '#fff8e1', border: '1px solid #ffe082', borderRadius: '12px', padding: '14px' }}>
              <div style={{ fontWeight: 700, fontSize: '13px', color: '#f57f17', marginBottom: '10px' }}>⚡ Vistar Quick Stats</div>
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '8px' }}>
                <div style={{ fontSize: '12px' }}><b>{vistar.total_sessions}</b> sessions</div>
                <div style={{ fontSize: '12px' }}><b>{vistar.fraud_flagged_count}</b> flagged</div>
                <div style={{ fontSize: '12px' }}>Avg Reported: <b>{vistar.avg_reported_attendance}</b></div>
                <div style={{ fontSize: '12px' }}>Avg Digital: <b>{vistar.avg_digital_attendance}</b></div>
              </div>
              <div style={{ marginTop: '10px', fontSize: '11px', color: '#888' }}>Gap: <b style={{ color: vistar.overall_gap_pct > 40 ? '#c62828' : '#2e7d32' }}>{vistar.overall_gap_pct}%</b></div>
            </div>
          )}
          ...
          {vistar?.insights?.length > 0 && (
            <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
              <div style={{ fontWeight: 700, fontSize: '13px' }}>🧠 Supervisory Insights</div>
      ...
      {!loading && activeTab === 'sahayak' && (
        <div style={{ display: 'flex', flexDirection: 'column', gap: '14px' }}>
          <SectionHeader icon="group" title="Sahayak Supervision" subtitle="Monitor field officer activity and workload" />
            ...
            return (
              <div key={sb.sahayak_id} style={{ background: '#fff', border: '1px solid #e0e0e0', borderRadius: '12px', padding: '14px' }}>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: '8px', fontSize: '12px' }}>
                  <div><div style={{ color: '#888', fontSize: '10px' }}>TOTAL APPS</div><b>{sb.total}</b></div>
                  <div><div style={{ color: '#888', fontSize: '10px' }}>PENDING</div><b style={{ color: '#e65100' }}>{sb.pending}</b></div>
                  <div><div style={{ color: '#888', fontSize: '10px' }}>APPROVED</div><b style={{ color: '#2e7d32' }}>{sb.approved}</b></div>
                </div>
                {vp && (
                  <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: '8px', fontSize: '12px', marginTop: '10px', paddingTop: '10px', borderTop: '1px solid #f0f0f0' }}>
                    <div><div style={{ color: '#888', fontSize: '10px' }}>SESSIONS</div><b>{vp.total_sessions}</b></div>
                    <div><div style={{ color: '#888', fontSize: '10px' }}>AVG ATTEND.</div><b>{vp.avg_digital_attendance}</b></div>
                    <div><div style={{ color: '#888', fontSize: '10px' }}>FRAUD FLAGS</div><b style={{ color: vp.fraud_flags > 0 ? '#c62828' : '#2e7d32' }}>{vp.fraud_flags}</b></div>
                  </div>
                )}
      ...
      {!loading && activeTab === 'apps' && (
        <div style={{ display: 'flex', flexDirection: 'column', gap: '14px' }}>
          <SectionHeader icon="analytics" title="Application Intelligence" subtitle="Supervisory view only — no approve/reject actions" />

          {/* Status funnel */}
          <div style={{ background: '#fff', border: '1px solid #e0e0e0', borderRadius: '12px', padding: '14px' }}>
            <div style={{ fontWeight: 700, fontSize: '13px', marginBottom: '10px' }}>Status Funnel</div>
              ...
              return (
                <div key={status} style={{ marginBottom: '8px' }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '12px', marginBottom: '3px' }}>
                    <span>{status}</span><span style={{ fontWeight: 700 }}>{count} ({pct}%)</span>
                  </div>
          ...
          {/* Scheme distribution */}
          <div style={{ background: '#fff', border: '1px solid #e0e0e0', borderRadius: '12px', padding: '14px' }}>
            <div style={{ fontWeight: 700, fontSize: '13px', marginBottom: '10px' }}>Scheme Category Distribution</div>
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '8px' }}>
              {Object.entries(appIntel?.by_scheme_category || {}).slice(0, 6).map(([cat, cnt]) => (
                <div key={cat} style={{ background: '#f8f9fa', borderRadius: '8px', padding: '8px 10px', fontSize: '12px' }}>
                  <div style={{ color: '#888', fontSize: '10px', marginBottom: '2px' }}>{cat || 'Other'}</div>
                  <b>{cnt}</b>
                </div>
              ))}
            </div>
          ...
          {(appIntel?.duplicate_alerts || []).length > 0 && (
            <div style={{ background: '#ffebee', border: '1px solid #ffcdd2', borderRadius: '12px', padding: '14px' }}>
              <div style={{ fontWeight: 700, fontSize: '13px', color: '#c62828', marginBottom: '10px' }}>
                🚨 Duplicate / Fraud Document Alerts ({appIntel.duplicate_alerts.length})
              </div>
      ...
      {!loading && activeTab === 'vistar' && (
        <div style={{ display: 'flex', flexDirection: 'column', gap: '14px' }}>
          <SectionHeader icon="school" title="Krushi Vistar Intelligence" subtitle="Training session attendance verification & fraud detection" />

          {/* Summary cards */}
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '10px' }}>
            <StatCard icon="event"        label="Total Sessions"   value={vistar?.total_sessions}         color="#0055A4" bg="#e3f2fd" />
            <StatCard icon="people"       label="Avg Digital Att." value={vistar?.avg_digital_attendance}  color="#2e7d32" bg="#e8f5e9" />
            <StatCard icon="report"       label="Fraud Flagged"    value={vistar?.fraud_flagged_count}     color="#c62828" bg="#ffebee" />
            <StatCard icon="trending_down" label="Attendance Gap"  value={`${vistar?.overall_gap_pct || 0}%`} color="#f57f17" bg="#fff8e1" />
          </div>

          {/* Sahayak performance table */}
          <div style={{ background: '#fff', border: '1px solid #e0e0e0', borderRadius: '12px', padding: '14px' }}>
            <div style={{ fontWeight: 700, fontSize: '13px', marginBottom: '12px' }}>Sahayak Vistar Performance</div>
                ...
                <div key={p.sahayak_id} style={{ display: 'grid', gridTemplateColumns: '1fr auto', alignItems: 'center', gap: '10px', padding: '10px', background: '#fafafa', borderRadius: '10px', border: '1px solid #f0f0f0' }}>
                  <div>
                    ...
                    <div style={{ fontSize: '11px', color: '#888', marginTop: '3px' }}>
                      {p.total_sessions} sessions · Reported avg: {p.avg_reported_attendance} · Digital avg: {p.avg_digital_attendance}
                      {p.fraud_flags > 0 && <span style={{ color: '#c62828', fontWeight: 700, marginLeft: '6px' }}>· {p.fraud_flags} HIGH flags</span>}
                    </div>
                    ...
                    <div style={{ fontSize: '10px', color: '#aaa', marginTop: '2px' }}>{Math.round(p.overall_compliance_ratio * 100)}% compliance</div>
                  </div>
              ...
              {fraudSes.map(s => (
                <div key={s.session_id} style={{ marginBottom: '10px', padding: '12px', background: s.risk === 'HIGH' ? '#ffebee' : '#fff8e1', borderRadius: '10px', border: `1px solid ${s.risk === 'HIGH' ? '#ffcdd2' : '#ffe082'}` }}>
                  ...
                  <div style={{ display: 'flex', gap: '20px', fontSize: '12px', marginBottom: '8px' }}>
                    ...
                    <span>📱 Digital: <b style={{ color: s.risk === 'HIGH' ? '#c62828' : '#f57f17' }}>{s.digital_attendance}</b></span>
          ...
          {/* Session timeline */}
          <div style={{ background: '#fff', border: '1px solid #e0e0e0', borderRadius: '12px', padding: '14px' }}>
            <div style={{ fontWeight: 700, fontSize: '13px', marginBottom: '12px' }}>Session Timeline</div>
            {fraudSes.concat(
              (vistar?.sahayak_performance || [])
                .filter(p => p.overall_risk === 'CLEAN')
                .map(p => ({ session_id: p.sahayak_id + '-clean', sahayak_name: p.sahayak_name, risk: 'CLEAN', date: '—', village: '—', topic: `${p.total_sessions} clean sessions`, gap_pct: 0 }))
            ).slice(0, 8).map((s, i) => (
