...
def main():
    ...
    old_index_path = os.path.join(frontend_dir, 'old_index.html')
    new_index_path = os.path.join(frontend_dir, 'new_index.html')
    output_index_path = os.path.join(frontend_dir, 'index.html')

    if not os.path.exists(old_index_path) or not os.path.exists(new_index_path):
        print(f"Error: Missing required files in {frontend_dir}")
        return
...
"""
    ...
    print(f"Successfully generated {output_index_path}")
